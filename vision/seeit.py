...
def draw_box(im, result, lables, threshold=0.5): 
    ...
    cropped_images = []  # List to save cropped images
    index= 0
    for i,dt in enumerate(result): 
        if dt["type"] == "table":
            ...
            (xmax, ymin), (xmin, ymin)], width=draw_thickness, fill=color) 
            # Crop the image inside the bounding box
            cropped_im = im.crop((xmin, ymin, xmax, ymax))
            cropped_images.append({"index":index,"page":i,"type":dt["type"],"bbox":dt["bbox"],"image":cropped_im,"confidence":dt["score"]})
            # draw label 
            text = "{} {:.4f}".format(dt["type"], dt["score"]) 
            tw, th = imagedraw_textsize_c(draw, text) 
            draw.rectangle([(xmin + 1, ymin - th), (xmin + tw + 1, ymin)], fill=color) 
            draw.text((xmin + 1, ymin - th), text, fill=(255, 255, 255)) 
    return im, cropped_images
